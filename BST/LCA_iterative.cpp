...
struct node* LCA_iterative(struct node* root,int n1, int n2)		// My solution using two while loops
{
	if(root==NULL)
		return(root);
	while(root->num > n1 && root->num > n2)
		root=root->left;
	while(root->num < n1 && root->num < n2)
		root=root->right;
	return(root);
...
struct node* LCA_iter(struct node* root,int n1, int n2)			// Geeks for Geeks iterative solution
{
	while(root!=NULL)
	{
		if(root->num > n1 && root->num > n2)
...
int main(void)
{
	...
	lca=LCA_iterative(root,30,45);
	printf("\n\n\n LCA_itrative is ->%d ",lca->num);

	lca=LCA_iter(root,30,45);
	printf("\n\n\n LCA_iter is ->%d ",lca->num);
